...
function generateHinges() {
  hinges = [];
  for (let i = 0; i < 5; i++) { // Adjust the number of hinges you want
    let posX = random(width);
    let posY = random(height);
    let angle1 = random(TWO_PI);
    let angle2 = random(PI);
    let length1 = random(50, 300); // Random length between 50 and 150
...
function drawHinges() {
  ...
  for (let i = 0; i < hinges.length; i++) {
    ...
    segment(0, 0, hinge.length1, hinge.angle1);
    stroke(hinge.color2); // Set color for second segment
    segment(hinge.length1, 0, hinge.length2, hinge.angle2);
    pop();
  }
